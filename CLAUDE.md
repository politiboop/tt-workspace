...
| `deep-dives/`                   | Multi-Page Dives    | Astro 6    | "Deep Dives" (working name) — one story per page in the election-rigging design language, built from tracker entries. `verify-sources.js` requires every link and every quotation to trace to a seed entry. Dives so far: the White House press ban (Shut Out), and a Corruption section of three questions answered by By His Own Account (profit, built line by line from his 2025 financial disclosure), The Ballroom (access) and Paid For (public money promoting the president). Has its own CLAUDE.md. |
